...
class Product(BaseModel):
    ...
    sort_by_date: bool = Field(default=False)
    model_config = ConfigDict(from_attributes=True)
...
class ReviewCreate(BaseModel):
    product_id: int = Field(..., description="id продукта, которому написан коммент")
    comment: str | None = Field(None, description="текст коммента")
    grade: int = Field(..., ge=1, le=5, description="рейтинг товара")


class Review(BaseModel):
    id: int = Field(...)
    user_id: int = Field(..., description="id пользователя, оставившего коммент")
    product_id: int = Field(..., description="id продукта, которому написан коммент")
    comment: str | None = Field(None, description="текст коммента")
